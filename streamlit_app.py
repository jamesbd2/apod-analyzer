...
if uploaded_file:
    ...
    if st.button("📄 Generate Investment Report PDF"):
        ...
        <html><body><h2>Investment Report</h2>
        <table border="1" cellspacing="0" cellpadding="5">
        <tr><th>Address</th><th>Price</th><th>Rent</th><th>NOI</th><th>Cap Rate</th><th>Cash Flow</th><th>CoC Return</th></tr>
        {% for p in properties %}
        <tr><td>{{ p.Address }}</td><td>${{ '{:,.0f}'.format(p.Price) }}</td>
        <td>${{ '{:,.0f}'.format(p.Rent) }}</td><td>${{ '{:,.0f}'.format(p.NOI) }}</td>
        <td>{{ '{:.2f}'.format(p.CapRate) }}%</td><td>${{ '{:,.0f}'.format(p.CashFlow) }}</td>
        <td>{{ '{:.2f}'.format(p.CoCReturn) }}%</td></tr>{% endfor %}
        </table></body></html>""")
